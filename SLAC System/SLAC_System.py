...
import os
from streamlit_drawable_canvas import st_canvas  # new import for signature pad
   ...
   if choice == "Check-In":
       ...
       if active.empty:
           ...
           if search and filtered.empty:
               ...
               # Step 4: Show checkout button and signature pad only after device is selected
               if selected:
                   ...
                   if stl.button("Confirm Check-Out"):
                       # MODIFICATION START
                       if canvas_result.json_data and any(element['path'] for element in canvas_result.json_data['objects']):
                           check_out(int(tx_id))
                           stl.success(f"Transaction {tx_id} checked out successfully.")
                       else:
                           stl.warning("Please provide your signature to confirm the check-out.")
                       # MODIFICATION END
